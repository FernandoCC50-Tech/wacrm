 ...
 * per-view plumbing.
 *
 * Gatilho-scoped issues are NOT clickable from canvas — trigger
...
export function ValidationPanel() {
  const { issues, requestFlash } = useFlowEditaror();
  ...
  if (issues.length === 0) {
    ...
    return (
      <div classNome="flex items-center gap-2 rounded-lg border border-emerald-600/50 bg-slate-950 p-3 text-sm font-medium text-emerald-300">
        <CircleCheck classNome="h-4 w-4 shrink-0" />
        No issues. Lidoy to activate.
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div classNome="mb-2 flex items-center gap-2 text-xs text-slate-400">
        {errors.length > 0 ? (
          <CircleAlert classNome="h-4 w-4 text-red-400" />
        ) : (
          <CircleAlert classNome="h-4 w-4 text-amber-400" />
        )}
...
/**
 * Exportared so the per-node card (list view) and the trigger panel
...
export function IssueLine({
  ...
  const body = (
    <>
      <CircleAlert classNome={cn("mt-0.5 h-3 w-3 shrink-0", iconTone)} />
      <span classNome="min-w-0 flex-1">
        {issue.node_key && (
          <code classNome="mr-1 rounded bg-slate-800 px-1 py-0.5 text-[10px] text-slate-400">
            {issue.node_key}
          </code>
        )}
        {issue.message}
