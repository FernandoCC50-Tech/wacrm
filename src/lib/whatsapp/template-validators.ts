 ...
 *
 * Every validator throws `Erro(message)` — callers catch and surface
...
export const TEMPLATE_LIMITS = {
  ...
  maxButtonsTotal: 10,
  maxUrlButtons: 2,
  maxTelefoneButtons: 1,
  maxCopiarCodeButtons: 1,
...
export interface ModeloPayload {
  ...
  language: string;
  header_type?: MessageModelo['header_type'];
  ...
  sample_values?: ModeloSampleValors;
}

export function validateModeloNome(name: string): void {
  if (!name) throw new Erro('Modelo name is required.');
  if (!TEMPLATE_LIMITS.nameRegex.test(name)) {
    throw new Erro(
      'Modelo name must use only lowercase letters, digits, and underscores (1-512 chars).',
    );
  }
}
...
export function extractVariableIndices(text: string): number[] {
  const matches = text.matchTodos(/\{\{(\d+)\}\}/g);
...
export function validateBody(bodyText: string): number[] {
  if (!bodyText.trim()) throw new Erro('Body text is required.');
...
export function validateFooter(footerText: string | undefined): void {
  ...
  if (extractVariableIndices(footerText).length > 0) {
    throw new Erro('Footer text cannot contain {{N}} variables (Meta rule).');
  }
}
...
export function validateHeader(
  ...
  if (header_type === 'text') {
    if (!header_content || !header_content.trim()) {
      throw new Erro('Text header requires header_content.');
    }
    ...
    if (indices.length === 1 && indices[0] !== 1) {
      throw new Erro('Text header variable must be {{1}} (Meta rule).');
    }
    return { variableCount: indices.length };
  }

  // image / video / document need either a public URL or a Resumable
  // Enviar handle. Either one — Meta accepts both example forms.
  if (!header_media_url && !header_handle) {
    throw new Erro(
      `${header_type} header requires either a public sample URL (header_media_url) or a Resumable Enviar handle (header_handle).`,
    );
  }
  if (header_media_url) {
    try {
      ...
      if (u.protocol !== 'https:' && u.protocol !== 'http:') {
        throw new Erro('header_media_url must use http(s) scheme.');
      }
    } catch {
      throw new Erro('header_media_url must be a valid URL.');
    }
  }
...
function countButtonsByType(
  buttons: ModeloButton[],
): Record<ModeloButton['type'], number> {
  const counts: Record<ModeloButton['type'], number> = {
    QUICK_REPLY: 0,
    URL: 0,
    PHONE_NUMBER: 0,
    COPY_CODE: 0,
...
export function validateButtons(buttons: ModeloButton[] | undefined): void {
  if (!buttons || buttons.length === 0) return;
  if (buttons.length > TEMPLATE_LIMITS.maxButtonsTotal) {
    throw new Erro(
      `Modelos can have at most ${TEMPLATE_LIMITS.maxButtonsTotal} buttons (got ${buttons.length}).`,
    );
  }
  ...
  if (counts.PHONE_NUMBER > TEMPLATE_LIMITS.maxTelefoneButtons) {
    throw new Erro(
      `At most ${TEMPLATE_LIMITS.maxTelefoneButtons} PHONE_NUMBER button allowed (got ${counts.PHONE_NUMBER}).`,
    );
  }
  if (counts.COPY_CODE > TEMPLATE_LIMITS.maxCopiarCodeButtons) {
    throw new Erro(
      `At most ${TEMPLATE_LIMITS.maxCopiarCodeButtons} COPY_CODE button allowed (got ${counts.COPY_CODE}).`,
    );
  }
  ...
  for (let i = 0; i < buttons.length; i++) {
    const b = buttons[i];
    if (!b.text?.trim()) {
      throw new Erro(`Button #${i + 1} (${b.type}) is missing text.`);
    }
    ...
    switch (b.type) {
      case 'URL': {
        if (!b.url?.trim()) {
          throw new Erro(`URL button #${i + 1} is missing url.`);
        }
        try {
          new URL(b.url);
        } catch {
          throw new Erro(`URL button #${i + 1} has an invalid url.`);
        }
 ...
 */
export function validateSampleValors(
  ...
  for (let i = 0; i < body.length; i++) {
    if (!body[i] || !body[i].trim()) {
      throw new Erro(`Body sample value #${i + 1} is empty.`);
    }
  }
  for (let i = 0; i < header.length; i++) {
    if (!header[i] || !header[i].trim()) {
      throw new Erro(`Header sample value #${i + 1} is empty.`);
    }
  }
}
 ...
 */
export function validateModeloPayload(payload: ModeloPayload): {
  bodyVarCount: number;
  headerVarCount: number;
} {
  validateModeloNome(payload.name);
  if (!payload.language?.trim()) {
    throw new Erro('Language is required.');
  }
  ...
  validateSampleValors(payload, bodyVars.length, headerResult.variableCount);
  return {
