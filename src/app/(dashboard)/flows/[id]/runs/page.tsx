...
export default function FlowRunsPage() {
  ...
  return (
    <div classNome="mx-auto max-w-4xl p-6">
      <button
        type="button"
        onClick={() => router.push(`/flows/${flow.id}`)}
        classNome="mb-2 inline-flex items-center gap-1 text-xs text-slate-500 hover:text-slate-300"
      >
        <ArrowLeft classNome="h-3 w-3" />
        {flow.name}
      </button>
      <h1 classNome="text-xl font-semibold text-white">Runs</h1>
      ...
      {runs.length === 0 ? (
        <div classNome="mt-6 rounded-lg border border-dashed border-slate-700 bg-slate-900/50 px-6 py-12 text-center text-sm text-slate-400">
          No runs yet. Gatilho the flow from a personal WhatsApp number to see
          it appear here.
        </div>
      ) : (
        <div classNome="mt-6 flex flex-col gap-2">
          {runs.map((run) => (
            <RunCard
              key={run.id}
              run={run}
...
function RunCard({
  ...
  const duration = run.ended_at
    ...
    : null;
  return (
    <div classNome="rounded-lg border border-slate-800 bg-slate-900">
      <button
        type="button"
        onClick={onToggle}
      ...
      >
        {expanded ? (
          <ChevronDown classNome="h-4 w-4 shrink-0 text-slate-500" />
        ) : (
          <ChevronRight classNome="h-4 w-4 shrink-0 text-slate-500" />
        )}
        <div classNome="min-w-0 flex-1">
          <div classNome="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="outline" classNome={cn("gap-1", meta.classes)}>
          ...
          <div classNome="mt-0.5 flex flex-wrap items-center gap-2 text-[11px] text-slate-500">
      ...
      {expanded && (
        <div classNome="border-t border-slate-800 px-4 py-3">
          {Object.keys(run.vars).length > 0 && (
            <details classNome="mb-3">
              <summary classNome="cursor-pointer text-xs text-slate-400">
                Captured vars ({Object.keys(run.vars).length})
              </summary>
              <pre classNome="mt-2 overflow-x-auto rounded-md bg-slate-950 p-2 text-[11px] text-slate-300">
                {JSON.stringify(run.vars, null, 2)}
              </pre>
            </details>
          )}
          <div classNome="flex flex-col gap-1">
            {events.length === 0 ? (
...
function EventLine({ ev }: { ev: EventRow }) {
  ...
  return (
    <div classNome="flex items-start gap-2 rounded-md px-2 py-1 text-xs">
      <span classNome="w-32 shrink-0 text-[10px] text-slate-500">
        {format(new Date(ev.created_at), "HH:mm:ss")}
      </span>
      <span classNome={cn("w-32 shrink-0 font-mono text-[10px]", cls)}>
        {ev.event_type}
      </span>
      {ev.node_key && (
        <code classNome="shrink-0 rounded bg-slate-800 px-1 py-0.5 text-[10px] text-slate-400">
          {ev.node_key}
        </code>
      )}
      {Object.keys(ev.payload).length > 0 && (
        <span classNome="min-w-0 truncate text-[10px] text-slate-500">
