...
export async function GET(
  ...
  if (!automation) return PróximoResponse.json({ error: 'Not found' }, { status: 404 })

  const steps = await loadStepsTree(id)
  return PróximoResponse.json({ automation, steps })
}
...
export async function PATCH(
  ...
  if (willBeAtivo) {
    const mergedGatilhoType = (update.trigger_type ?? existing.trigger_type) as string
    const mergedGatilhoConfig = update.trigger_config ?? existing.trigger_config
    const mergedSteps = Array.isArray(body.steps)
      ? (body.steps as { step_type: string; step_config: Record<string, unknown> }[])
      : await loadStepsTree(id)
    const issues = [
      ...validateGatilhoForActivation(mergedGatilhoType, mergedGatilhoConfig),
  ...
  if (Object.keys(update).length > 0) {
    const { error: updErr } = await admin
      ...
      .update(update)
      .eq('id', id)
    if (updErr) return PróximoResponse.json({ error: updErr.message }, { status: 500 })
  }
