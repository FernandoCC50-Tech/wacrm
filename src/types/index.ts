...
/**
 * Hydrated member row for the Settings → Membros tab. Combines
...
export interface AccountInvitation {
  id: string;
  account_id: string;
  /** Funçãos offered via invite — owner is never offered. */
  role: Exclude<AccountFunção, "owner">;
...
export interface Contact {
   ...
   *  and unique per account. Lido-only. */
...
export interface ContactCustomValor {
  id: string;
  contact_id: string;
...
export type EnviarerType = 'customer' | 'agent' | 'bot';
...
export type MessageModeloStatus =
  | 'DRAFT'
  | 'PENDING'
  | 'APPROVED'
  | 'REJECTED'
...
export type ModeloButton =
  | { type: 'QUICK_REPLY'; text: string }
  | { type: 'URL'; text: string; url: string; example?: string }
  | { type: 'PHONE_NUMBER'; text: string; phone_number: string }
  | { type: 'COPY_CODE'; text: string; example: string };

export interface ModeloSampleValors {
  body?: string[];
  header?: string[];
}

export interface MessageModelo {
  id: string;
  user_id: string;
  name: string;
...
// ============================================================
// Automações (migration 006)
// ============================================================

export type AutomationGatilhoType =
...
export interface KeywordMatchGatilhoConfig {
  keywords: string[];
  match_type: 'exact' | 'contains';
  case_sensitive?: boolean;
}

export interface TagGatilhoConfig {
  tag_id: string;
}

export interface TimeBasedGatilhoConfig {
  /** Cron expression or simple HH:mm string; engine can accept either. */
  schedule: string;
  timezone?: string;
}

export type AutomationGatilhoConfig =
  | Record<string, never>
  | KeywordMatchGatilhoConfig
  | TagGatilhoConfig
  | TimeBasedGatilhoConfig
  | Record<string, unknown>;

export interface EnviarMessageStepConfig {
  text: string;
}

export interface EnviarModeloStepConfig {
  template_name: string;
  language?: string;
...
export interface CriarDealStepConfig {
  pipeline_id: string;
  stage_id: string;
  title: string;
  value?: number;
...
export type CondiçãoSubject =
  | 'contact_field'
  | 'tag_presence'
  | 'message_content'
  | 'time_of_day';

export interface CondiçãoStepConfig {
...
export interface EnviarWebhookStepConfig {
  url: string;
